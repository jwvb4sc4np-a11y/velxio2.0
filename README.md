...
**Prerequisites:** [Bun](https://bun.sh) 1.0+, Python 3.12+

> **`arduino-cli` is installed automatically** — no manual setup needed.
> The `bun install` step runs a `postinstall` script that detects your OS
> and installs + configures `arduino-cli` and all required board cores.

```bash
git clone https://github.com/davidmonterocrespo24/velxio.git
cd velxio

# Installs all JS dependencies AND arduino-cli + board cores automatically
bun install
```

This single command:
1. Installs all frontend and workspace dependencies
2. Runs `scripts/setup-arduino-cli.mjs` which:
   - **Windows** → installs via `winget install ArduinoSA.CLI`
   - **macOS** → installs via `brew install arduino-cli` (or official script)
   - **Linux / VPS** → installs via the official script to `/usr/local/bin` (fallback: `~/.local/bin`)
3. Installs and configures the required board cores:
   - `arduino:avr` — Arduino Uno / Nano / Mega
   - `rp2040:rp2040` — Raspberry Pi Pico / Pico W
   - `ATTinyCore:avr` — ATtiny85
...
> **Linux note:** If `arduino-cli` was installed to `~/.local/bin` (no sudo),
> add it to your PATH permanently:
> ```bash
> echo 'export PATH="$HOME/.local/bin:$PATH"' >> ~/.bashrc && source ~/.bashrc
> ```

> **Windows note:** After a fresh install, if the backend reports
> `arduino-cli not found`, the backend auto-resolves the path from known
> winget install locations — no restart needed.

> **Re-run arduino-cli setup manually** at any time:
> ```bash
> bun run setup:arduino
> ```
...
**`arduino-cli: command not found`** — run `bun install` from the repo root. The `postinstall` script installs `arduino-cli` automatically for your OS. Alternatively run `bun run setup:arduino` directly.

**Backend returns 500 on compile (Windows)** — `arduino-cli` was installed after `uvicorn` started. The backend auto-resolves the binary path from known winget locations without a restart. If it still fails, stop uvicorn, open a new terminal (so PATH updates), and start it again.
...
**Compilation errors** — check the compilation console; verify the correct core is installed for the selected board. Run `bun run setup:arduino` to reinstall all cores.
